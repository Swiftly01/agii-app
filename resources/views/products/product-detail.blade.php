    ...
    <div class="container-fluid py-5">
        <div class="row">
            ...
            <div class="col-lg-6">
                ...
                <!-- Action Buttons -->
                <div class="action-buttons d-grid gap-2 d-md-flex mb-3">
                        ...
                        <div class="tab-pane fade" id="reviews" role="tabpanel">
                            <div class="reviews-summary d-flex align-items-center mb-4">
                                <div class="text-center me-4">
                                    <div class="display-5 fw-bold">{{ number_format($product->rating, 1) }}</div>
                            ...
                            @auth
                                @if (Auth::user()->isCustomer())
                                    <div class="review-form-card border rounded p-3 mb-4">
                                        ...
                                        <form method="POST" action="{{ route('reviews.store', $product->id) }}">
                                            ...
                                            <div class="mb-3">
                                                <textarea name="comment" class="form-control" rows="3"
                                                    placeholder="Share your experience with this product (optional)">{{ old('comment', $userReview->comment ?? '') }}</textarea>
                                            </div>
                                            <button type="submit" class="btn btn-success btn-sm">
                                                {{ $userReview ? 'Update Review' : 'Submit Review' }}
                                            </button>
                                            @if ($userReview)
                                                <button type="submit" form="deleteReviewForm" class="btn btn-outline-danger btn-sm">Delete Review</button>
                                            @endif
                                        </form>
                                        @if ($userReview)
                                            <form id="deleteReviewForm" method="POST"
                                                action="{{ route('reviews.destroy', $userReview->id) }}" class="d-none">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        @endif
                                ...
                                @else
                                    <div class="alert alert-light border small mb-4">Only customers can leave a review.</div>
                                @endif
                            @else
                                <div class="alert alert-light border small mb-4">
                                    <a href="{{ route('login') }}">Sign in</a> as a customer to leave a review.
                                </div>
                            @endauth
                            ...
                            @forelse ($product->reviews as $review)
                                <div class="review-item border-bottom pb-3 mb-3">
                                    <div class="d-flex justify-content-between">
                                        <div class="fw-bold">{{ trim(($review->user->first_name ?? 'Customer') . ' ' . ($review->user->last_name ?? '')) }}</div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.contact-seller').forEach(button => {
             ...
             // COOKIE FUNCTIONS
        function setViewCookie(productId) {
        ...
        function hasViewedRecently(productId) {
            const cookieName = `product_view_${productId}`;
