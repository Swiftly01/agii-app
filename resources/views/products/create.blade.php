                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="productLocation" name="location"
                                            value="{{ Auth::user()->local_government }}"
                        ...
                        <div class="dropzone-container">
                            ...
                            <div class="dropzone" onclick="document.getElementById('imageUpload').click()">
                                <div class="dz-message">
                                    ...
                                    <p class="text-muted">Upload at least 5 photos, up to 10 (max 5MB each)</p>
                                </div>
                            </div>
                        </div>
...
alert(`Please upload at least 5 photos. You currently have ${uploadedImages.length}.`);
                    isValid = false;
                }
            }
        ...
        document.getElementById('productForm').addEventListener('submit', function(e) {
            ...
            // Add specifications as individual fields (Laravel will convert to array)
            const specifications = {};
            Object.keys(specificationFields).forEach(key => {
                const fieldElement = document.getElementById(`spec_${key}`);
                if (fieldElement && fieldElement.value) {
                    // Add each specification as a separate form field
                    formData.append(`specifications[${key}]`, fieldElement.value);
                }
            });

            // Alternative approach: Add specifications as JSON string (if above doesn't work)
            // const specifications = {};
            // Object.keys(specificationFields).forEach(key => {
            //     const value = document.getElementById(`spec_${key}`)?.value;
            //     if (value) {
            ...
            fetch('{{ route('products.store') }}', {
                ...
                .catch(error => {
                    ...
                    alert('An error occurred while publishing your product: ' + error.message);
                    submitBtn.disabled = false;
                    submitBtn.innerHTML = 'Publish Product';
                });
        });
